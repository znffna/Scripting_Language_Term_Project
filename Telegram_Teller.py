...
class Telegram:
    ...
    def replyDayMatchData(self, date_param, user):
    ...
    def handle(self, msg):
        ...
        if text.startswith('오늘의경기'):
            self.replyDayMatchData(int(now.year * 10000 + now.month * 100 + now.day), chat_id)
        elif text.startswith('경기') and len(args) > 1:
            print('try to selected day 경기', args[1])
            self.replyDayMatchData(args[1], chat_id)
        elif text.startswith('저장') and len(args) > 1:
            print('try to 저장', args[1])
            self.save(chat_id, args[1])
    ...
    def __init__(self):
            ...
            self.bot = telepot.Bot(Telegram_bot.TOKEN)
